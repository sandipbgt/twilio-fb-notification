...
def write_notifications_to_file(notifs):
	...
	with open('notifications.csv', mode='w', encoding='utf-8') as notif_file:
		...
		for notif in notifs:
			notif_csv_writer.writerow([notif['guid'], notif['title'], notif['pub_date'], notif['sent']])
...
params = urllib.parse.urlencode({'id': user_id, 'viewer': user_id, 'key': key, 'format': 'rss20'})
